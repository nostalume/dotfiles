...
4. Choose a node in the `节点选择` group, or select `DIRECT`.
...
```text
seven source-named proxy-providers
→ source prefixing
→ GEOSITE,CN,DIRECT and GEOIP,CN,DIRECT
→ one manual 节点选择 group (plus DIRECT)
→ MATCH,节点选择
```

China-mainland domains use `GEOSITE,CN,DIRECT` and China-mainland destination IPs use `GEOIP,CN,DIRECT,no-resolve`, both before the catch-all rule. All remaining traffic therefore uses the selected `节点选择` member. This routing policy does not remove subscription nodes by name. DNS, TUN, rule providers, health tests, and GUI remote-profile automation are outside this profile and must be added deliberately.
